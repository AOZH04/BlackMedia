(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".answer__slider")) new core(".answer__slider", {
            modules: [ Navigation, Pagination, Scrollbar, Keyboard, Mousewheel, Autoplay, EffectFade, EffectFlip, EffectCube, EffectCoverflow, Grid ],
            observer: true,
            observeParents: true,
            observeSlideChildren: true,
    ...
    if (customSelect) {
        let radios = customSelect.querySelector(".radios");
        let list = customSelect.querySelector(".list");
        let cityspans = document.querySelectorAll(".cityspan");
        for (let index = 0; index < radios.children.length; index++) {
            const element = radios.children[index];
            list.insertAdjacentHTML("beforeend", `<li><a target="blank" href="${element.dataset.link}"><label for="${element.id}">${element.title}</label></a></li>`);
            for (let index = 0; index < cityspans.length; index++) {
                const cityspan = cityspans[index];
                if (element.hasAttribute("checked")) cityspan.insertAdjacentHTML("beforeend", `${element.title}`);
            }
        }
        customSelect.addEventListener("click", (function() {
            for (let index = 0; index < list.children.length; index++) {
                const parentLabel = list.children[index];
                let label = parentLabel.querySelector("label");
                parentLabel.addEventListener("click", (function() {
                    for (let index = 0; index < cityspans.length; index++) {
                        const cityspan = cityspans[index];
                        cityspan.innerHTML = label.innerHTML;
                    }
    ...
    let DataScroll = document.querySelector("[data-scroll]");
    let pageMain = document.querySelector(".main_page");
    if (pageMain) DataScroll.setAttribute("data-scroll", "0");
    document.addEventListener("DOMContentLoaded", (() => {
        function setScrollBy() {
            window.scroll(0, 1);
        }
        if (pageMain) setScrollBy();
    }));
    let imgParallax = document.querySelector(".top_bg_img8");
    if (imgParallax) if (window.screen.width < 768) {
        imgParallax.removeAttribute("data-prlx-mouse");
        imgParallax.removeAttribute("data-prlx-cy");
        imgParallax.removeAttribute("data-prlx-dxr");
        imgParallax.removeAttribute("data-prlx-a");
        imgParallax.style.transform = "translate3d(0,-100%,0)";
    }
    window["FLS"] = false;
    menuInit();
    functions_menuClose();
